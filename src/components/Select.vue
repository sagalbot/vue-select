...
export default {
  ...
  props: {
    ...
    /**
     * Enable/disable the permission to create identical duplicate options.
     * @type {Boolean}
     */
    duplicate: {
      type: Boolean,
  ...
  methods: {
     ...
     */
    select(option) {
      // If duplicate is allowed, overwrite the check for "isOptionSelected"
      if (!this.isOptionSelected(option) || this.duplicate) {
        // Bug: "optionExists" doesn't work for multiple object tagging
