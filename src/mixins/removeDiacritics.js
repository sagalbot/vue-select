module.exports = {
	...
	created: function() {
		/**
		 * Create the diacritics map that will replace the
		 * given unicode character with its non-accented
		 * version
		 */
		for (let i = 0; i < this.defaultDiacriticsMap.length; i += 1) {
		  for (let j = 0; j < this.defaultDiacriticsMap[i].letters.length; j += 1) {
		    this.diacriticsMap[this.defaultDiacriticsMap[i].letters[j]] = this.defaultDiacriticsMap[i].base;
		  }
		}
	},
